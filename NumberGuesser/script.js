...
const compareGuesses = (userGuess, computerGuess, targetGuess) => {
  ...
  const userDifferance = getAbsoluteDistance(userGuess, targetGuess);
  const computerDifferance = getAbsoluteDistance(computerGuess, targetGuess);

  return userDifferance <= computerDifferance;
};
...
7.
Great work! Visit our forums to compare your project to our sample solution code. You can also learn how to host your own solution on GitHub 
so you can share it with other learners! Your solution might look different from ours, and that’s okay! There are multiple ways to solve these projects, 
